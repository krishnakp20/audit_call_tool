...
export default function CallAuditLogPage() {
  ...
  const [outcome, setOutcome] = useState<string>("All outcomes");
  const [lead, setLead] = useState<string>("All leads");
  const [score, setScore] = useState<string>("All scores");
  ...
  const { data, isLoading } = useQuery<CallAuditResponse>({
  ...
  const rows: CallAuditRow[] = data?.data || [];

  /* ================= AGENT LIST ================= */

  const agentList = useMemo<string[]>(() => {
  ...
  const filteredRows = useMemo<CallAuditRow[]>(() => {
